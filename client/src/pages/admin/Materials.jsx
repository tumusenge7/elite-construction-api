  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#1a3a5c]">Materials</h1>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>{['Name', 'Category', 'Unit', 'Unit Cost', ''].map(h => <th key={h} className="text-left px-4 py-3 font-semibold text-gray-600">{h}</th>)}</tr>
            </thead>
            <tbody>
              {items.length === 0 ? <tr><td colSpan={5} className="text-center py-12 text-gray-400">No materials found</td></tr>
                : items.map(m => (
                  <tr key={m._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-700">{m.unitCost ? `$${Number(m.unitCost).toLocaleString()}` : '—'}</td>
                    <td className="px-4 py-3"><button onClick={() => del(m._id)} className="text-red-400 hover:text-red-600"><Trash2 size={15} /></button></td>
                  </tr>
                ))}
            </tbody>
          </table>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-md p-6 space-y-4">
            <h2 className="font-bold text-[#1a3a5c]">Add Material</h2>
            <form onSubmit={save} className="space-y-3">
              {[['name','Name',true],['category','Category'],['unit','Unit (e.g. kg, m², pcs)'],['unitCost','Unit Cost'],['description','Description']].map(([k,l,req]) => (
                <div key={k}>
